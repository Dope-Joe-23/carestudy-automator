...
def _add_draft(doc, draft, skip_tables=False):
    ...
    skip_tables drops table blocks instead of rendering them — used when the
    section's structured rows already render its table, so the document never
    shows two tables with the same content."""
    lines = draft.split("\n")
    index = 0
...
def _add_toc(doc, chapters):
    ...
    for chapter_index, chapter in enumerate(chapters):
        ...
        _add_run(p, f"CHAPTER {ROMAN[chapter_index]}: {chapter['name'].upper()}", bold=True)
...
def _render_section(doc, section):
    ...
    # Sections may carry drafts, fields AND tables (e.g. 5.1) — render each
    # independently rather than as an either/or chain. When structured row data
    # is present it is the section's authoritative table, so any table inside
    # the draft is skipped to avoid duplicating it.
    if draft:
        ...
        _add_draft(doc, draft, skip_tables=bool(row_data))
    elif fields:
        for field in fields:
            p = doc.add_paragraph()
            p.paragraph_format.space_after = Pt(2)
            _add_run(p, f"{field.get('label', '')}: ", bold=True)
            _add_inline_text(p, _format_ordinal_dates(field.get("value", "")))
    if row_data:
...
def _add_chapter(doc, chapter_index, chapter, include_intro=True):
    ...
    _add_run(
        chapter_heading,
        f"CHAPTER {ROMAN[chapter_index]}: {chapter.get('name', '').upper()}",
        bold=True,
        size=Pt(14),
    )
...
def build_docx(payload):
    ...
    if scope_type == "full":
        ...
        for chapter_index, chapter in enumerate(chapters):
            _add_chapter(doc, chapter_index, chapter)
