...
SECTIONS = {
    ...
    "2.1 Comparison of Data with Standards": {
        "blurb": "Patient findings compared against the literature reviewed using subsections (Causes, Clinical Features, Treatment, Complications) and comparison tables.",
        "fields": [
            _f("introDefinition", "Definition of medical tests/investigations (with citation)",
               "e.g. A medical test is a procedure performed to detect, diagnose, or monitor diseases... (Bolboacă, 2019)",
               "textarea",
               hint="Opening definition with in-text citation from reference material."),
            _f("investigationsList", "List of diagnostic investigations carried out on patient",
               "e.g. Chest x-ray, FBC, Urinalysis, ESR, Sickling test",
               "textarea",
               hint="List each investigation performed on the patient during admission."),
            _f("testsComparisonComment", "Commentary on % of tests compared to literature",
               "e.g. About 67% of diagnostic tests carried out on patient were found in the literature; malaria parasites was not in literature but was requested.",
               "textarea",
               hint="State how many of the patient's tests match the literature standards."),
            _f("causesComparison", "Causes: literature vs patient's contributing factors",
               "e.g. Literature: bacterial, viral, aspiration. Patient: poor ventilation, dust exposure, weakened immunity.",
               "textarea",
               hint="Compare literature causes with the specific factors contributing to THIS patient's condition."),
            _f("clinicalFeaturesComparison", "Clinical features: patient vs literature (with table)",
               "e.g. Table showing each feature: present/absent in patient compared to literature list",
               "textarea",
               hint="List each clinical feature from literature and state whether the patient exhibited it."),
            _f("clinicalFeaturesComment", "Commentary on % of clinical features exhibited",
               "e.g. Patient exhibited about 57% of the signs and symptoms found in literature.",
               "textarea",
               hint="State the percentage or proportion of clinical features the patient showed."),
            _f("treatmentGivenList", "List of treatment/medications given to patient",
               "e.g. IV Ceftriaxone 2g daily, IV Paracetamol 1g tds, Tab Azithromycin 500mg od",
               "textarea",
               hint="List each drug or treatment the patient received."),
            _f("treatmentComparison", "Treatment: patient vs standard (with table and comments)",
               "e.g. Table comparing each drug: patient's drug | standard drug | match/comments",
               "textarea",
               hint="Compare each treatment given with standard treatment from literature."),
            _f("treatmentComment", "Commentary on % of drugs from literature given",
               "e.g. About 75% of drugs found in literature were given to the patient.",
               "textarea",
               hint="State what proportion of standard treatment the patient received."),
            _f("complicationsComparison", "Complications: literature vs patient status",
               "e.g. Literature: pleural effusion, septicaemia, respiratory failure. Patient: No complications observed.",
               "textarea",
               hint="List possible complications from literature and state whether the patient developed any."),
        ],
        "rows": None,
    },
    ...
    # Chapter 2 sections 2.3, 2.4, 2.5 — NMC standard format
    # These sections present problems, strengths, and nursing diagnoses
    # as bulleted lists (not prose), directly derived from Chapter 1 data.
    "2.3 Health Problems Identified": {
        "blurb": "Actual and potential health problems prioritised from the assessment data in Chapter 1.",
        "fields": [
            _f("actualProblems", "Actual health problems (from presenting complaints & current assessment)",
               "e.g. Dizziness, palpitation, headache, general body weakness, anxiety, insomnia",
               "textarea",
               hint="List problems identified from the patient's presenting symptoms, physical assessment, and history."),
            _f("potentialProblems", "Potential health problems / risks (if any)",
               "e.g. Risk for falls, risk for decreased cardiac output",
               "textarea",
               hint="Identify risks the patient may develop based on their condition."),
            _f("problemPriority", "Priority of problems (if prioritised)",
               "e.g. High priority: uncontrolled BP, risk for stroke; Medium: anxiety, insomnia",
               "textarea",
               hint="Optional: order problems by urgency using Maslow's hierarchy or ABC framework."),
        ],
        "rows": None,
    },
    "2.4 Patient/Family Strengths": {
        "blurb": "Strengths that support the patient's care and recovery, mapped to identified problems.",
        "fields": [
            _f("generalStrengths", "General strengths (family support, coping, resources)",
               "e.g. Supportive family, religious faith, positive outlook, financial support from children",
               "textarea",
               hint="Strengths not tied to a specific problem — general resilience factors."),
            _f("specificStrengths", "Specific strengths that counter identified problems",
               "e.g. Patient can verbalize pain intensity; Patient able to anticipate dizziness; Patient willing to learn",
               "textarea",
               hint="For each identified problem, note a corresponding strength that mitigates it."),
        ],
        "rows": None,
    },
    "2.5 Nursing Diagnoses (NANDA-I)": {
        "blurb": "NANDA-I nursing diagnoses derived from the health problems — each in 'Problem related to Etiology' format.",
        "fields": [
            _f("nursingDiagnoses", "Nursing diagnoses (NANDA-I format)",
               "e.g. Risk for falls related to dizziness; Acute pain related to increased cerebrovascular resistance; Knowledge deficit related to inadequate exposure to information about hypertension",
               "textarea",
               hint="Use NANDA-I format: Diagnosis label related to (r/t) etiology. Derive from the problems identified in 2.3."),
            _f("diagnosisPriority", "Priority ranking of diagnoses",
               "e.g. 1. Decreased cardiac output; 2. Acute pain; 3. Anxiety; 4. Knowledge deficit",
               "textarea",
               hint="Order diagnoses by clinical urgency (ABC, Maslow). This informs the care plan in Chapter 3."),
        ],
        "rows": None,
    },
...
WORD_COUNT_RANGES: dict = {
    ...
    "nursing diagnoses": (150, 400),
    "nursing diagnoses (nanda-i)": (150, 400),
