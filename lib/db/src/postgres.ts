...
-- Students: add staff_id column to tie students to staff members
ALTER TABLE "students" ADD COLUMN IF NOT EXISTS "staff_id" integer;

-- Student invites: one-time registration tokens generated by staff/admins
...
export function createPostgresStore(): StudyStore {
  ...
  return {
    ...
    async listOrdersByStaff(staffId) {
      ...
      // For Postgres, we can query each student ID individually or filter in-memory
      const rows = await db.select().from(orders).orderBy(desc(orders.id));
