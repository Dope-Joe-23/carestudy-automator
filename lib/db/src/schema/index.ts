...
export const staffInvitesTable = sqliteTable("staff_invites", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
export const studentOrdersTable = sqliteTable(
  "student_orders",
  {
    ...
    paymentStatus: text("payment_status").notNull().$defaultFn(() => "none"),
...
/** Student invite links — one-time registration tokens generated by staff/admins. */
export const studentInvitesTable = sqliteTable("student_invites", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  /** Unique invite token (URL-safe). */
  token: text("token").notNull().unique(),
